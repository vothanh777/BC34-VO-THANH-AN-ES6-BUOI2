...
const loadGlasses = () => {
  ...
  dataGlasses.forEach((glass) => {
    content += `
        <div class="vglasses__items col-4" id="${glass.id}">
...
window.removeGlasses = (isTrue) => {
  if (isTrue) {
    ...
    let glasses = document.querySelectorAll(".vglasses__items");
    glasses.forEach((item) => {
      item.addEventListener("click", () => {
        let data = dataGlasses.find((glass) => glass.id == item.id);
        document.getElementById("avatar").innerHTML = `
      <img src=${data.virtualImg}>
    `;
        document.getElementById("glassesInfo").innerHTML = `
      <h4>${data.name} - ${data.brand} (${data.color})</h4>
      <p><span class="price">$${data.price}</span> - Stocking</p>
